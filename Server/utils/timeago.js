export const timeAgo = (date) => {
    ...
    if ((interval = Math.floor(seconds / 31536000)) >= 1) return `${interval} yeat${interval > 1 ? '' : ''}ago`;
    ...
    if ((interval = Math.floor(seconds / 60)) >= 1) return `${interval} minutes${interval > 1 ? 's' : ''}ago`;
    
    return `${seconds} second${seconds !== 1 ? 's' : ''} ago`;
};


export const timeAgoMessage = (date) => {
    const seconds = Math.floor((new Date() - new Date(date)) / 1000);
    let interval;
